...
def set_toxicity_categories(data):
    category_mapping = {
        ...
        'ruralair': [('air',), ('air', 'low population density, long-term'), ('air', 'non-urban air or from high stacks')],
        'seawater': [('water', 'ocean')],
        'urbanair': [('air', 'urban air close to ground'), ('air', 'indoor'), ('air', 'urban air close to ground')],
    }
    ...
    for ds in data:
        ...
        for cf in ds["exchanges"]:
            try:
                categories = category_mapping[cf['compartment'].replace(" ", "").lower()]
                to_remove.append(cf)
                to_add.extend(
                    [set_categories(deepcopy(cf), cat) for cat in categories]
                )
            except KeyError:
                continue
